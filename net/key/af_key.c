					      ...
					      int add_keys, int hsc)
{
	...
	if (hsc & 2) {
		...
		lifetime->sadb_lifetime_allocations =  _X2KEY(x->sx[0].lft.hard_packet_limit);
		lifetime->sadb_lifetime_bytes = _X2KEY(x->sx[0].lft.hard_byte_limit);
		lifetime->sadb_lifetime_addtime = x->sx[0].lft.hard_add_expires_seconds;
		lifetime->sadb_lifetime_usetime = x->sx[0].lft.hard_use_expires_seconds;
	}
	/* soft time */
	if (hsc & 1) {
		lifetime = skb_put(skb, sizeof(struct sadb_lifetime));
		...
		lifetime->sadb_lifetime_allocations =  _X2KEY(x->sx[0].lft.soft_packet_limit);
		lifetime->sadb_lifetime_bytes = _X2KEY(x->sx[0].lft.soft_byte_limit);
		lifetime->sadb_lifetime_addtime = x->sx[0].lft.soft_add_expires_seconds;
		lifetime->sadb_lifetime_usetime = x->sx[0].lft.soft_use_expires_seconds;
	}
	/* current time */
	lifetime = skb_put(skb, sizeof(struct sadb_lifetime));
	lifetime->sadb_lifetime_len =
	...
	lifetime->sadb_lifetime_allocations = x->sx[0].curlft.packets;
	lifetime->sadb_lifetime_bytes = x->sx[0].curlft.bytes;
	lifetime->sadb_lifetime_addtime = x->sx[0].curlft.add_time;
	lifetime->sadb_lifetime_usetime = x->sx[0].curlft.use_time;
	/* src address */
