...
static int esp_output_head(struct xfrm_state *x, struct sk_buff *skb, __u8 proto, int tfclen, int tailen, int plen, bool *inplace)
{
	struct ip_esp_hdr *esph;
...
static int esp_output_tail(struct xfrm_state *x, struct sk_buff *skb, __be64 seqno, int clen, int nfrags, bool inplace)
{
	struct esp_output_extra *extra;
	int err = -ENOMEM;
	...
	esph = esp_output_set_extra(skb, x, ip_esp_hdr(skb), extra);
...
static int esp_output(struct xfrm_state *x, struct sk_buff *skb)
{
	...
	*skb_mac_header(skb) = IPPROTO_ESP;
	...
	return esp_output_tail(x, skb, seqno, clen, nfrags, inplace);
}
