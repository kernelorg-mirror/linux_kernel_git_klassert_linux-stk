...
#define for_each_xfrmi_rcu(start) \
...
struct xfrm_if *xfrmi_lookup(struct net *net, struct xfrm_state *x)
{
	...
	for_each_xfrmi_rcu(xfrmn->xfrmi[0]) {
	if (((x->props.input_mark & xi->p.imask) == xi->p.imark) &&
	    (xi->dev->flags & IFF_UP))
			return xi;
	}
...
static struct xfrm_if *xfrmi_create(struct net *net, struct xfrm_if_parms *p)
{
	...
	xi->net = net;
	xi->dev = dev;
	xi->phydev = __dev_get_by_index(net, p->link);
				   ...
				   int create)
{
	...
	for (xip = &xfrmn->xfrmi[0];
	     ...
	     xip = &xi->next) {
		if (((xi->p.imark & xi->p.imask) == (p->imark & p->imask)) ||
		    ((xi->p.omark & xi->p.omask) == (p->omark & p->omask))) {
			if (create)
				return NULL;
...
xfrmi_xmit2(struct sk_buff *skb, struct net_device *dev, struct flowi *fl)
{
	...
	if ((x->props.output_mark & xi->p.omask) != xi->p.omark)
		goto tx_err_link_failure;
	...
	skb_scrub_packet(skb, !net_eq(xi->net, dev_net(dev)));
	skb_dst_set(skb, dst);
			       ...
			       struct xfrm_if_parms *parms)
{
	...
	if (data[IFLA_XFRM_IMARK])
		parms->imark = nla_get_u32(data[IFLA_XFRM_IMARK]);

	if (data[IFLA_XFRM_IMASK])
		parms->imask = nla_get_u32(data[IFLA_XFRM_IMASK]);

	if (data[IFLA_XFRM_OMARK])
		parms->omark = nla_get_u32(data[IFLA_XFRM_OMARK]);

	if (data[IFLA_XFRM_OMASK])
		parms->omask = nla_get_u32(data[IFLA_XFRM_OMASK]);
}
...
static int xfrmi_fill_info(struct sk_buff *skb, const struct net_device *dev)
{
	...
	if (nla_put_u32(skb, IFLA_XFRM_LINK, parm->link) ||
	    nla_put_u32(skb, IFLA_XFRM_IMARK, parm->imark) ||
	    nla_put_u32(skb, IFLA_XFRM_IMASK, parm->imask) ||
	    nla_put_u32(skb, IFLA_XFRM_OMARK, parm->omark) ||
	    nla_put_u32(skb, IFLA_XFRM_OMASK, parm->omask))
...
static const struct nla_policy xfrmi_policy[IFLA_XFRM_MAX + 1] = {
	...
	[IFLA_XFRM_IMARK]	= { .type = NLA_U32 },
	[IFLA_XFRM_IMASK]	= { .type = NLA_U32 },
	[IFLA_XFRM_OMARK]	= { .type = NLA_U32 },
	[IFLA_XFRM_OMASK]	= { .type = NLA_U32 },
};
